...
import { transformSearchResult, JACKETT_CATEGORIES, } from '../types/search.types.js';
class JackettService {
    ...
    async checkConnection() {
        try {
            const response = await fetch(`${this.baseUrl}/api/v2.0/indexers/all/results?apikey=${this.apiKey}&Query=test`, { method: 'GET' });
            return response.ok;
        }
        catch (error) {
    ...
    async getIndexers() {
        try {
            const response = await fetch(`${this.baseUrl}/api/v2.0/indexers?apikey=${this.apiKey}`, { method: 'GET' });
            if (!response.ok) {
    ...
    async search(params) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}${endpoint}?${queryParams.toString()}`, { method: 'GET' });
            if (!response.ok) {
                throw new Error(`Jackett API error: ${response.status}`);
            }
    ...
    // Search for movies
    async searchMovies(query, limit) {
